...
//		frameLayout.addView(img, new FrameLayout.LayoutParams(newBitmapWidth, newBitmapHeight));
		...
		setContentView(R.layout.fragment_render);
//		enableFullscreen();
...
//					File appDirectory = new File(Environment.getExternalStorageDirectory().getPath() + "/Android/data/" + getPackageName() + "/files/models");
					...
					if(content != null){
						...
						bundle.putString(FileExplorerFragment.texToParse, "textures_disabled");
						String type = content.substring(content.indexOf(".")+1);
						Log.d("type", type);
						bundle.putString("fileType", type);
//						renderFrag.sendToRender(bundle);
						renderFrag.setArguments(bundle);
					}
