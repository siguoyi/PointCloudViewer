...
public class Mesh {
	...
	private FloatBuffer meshVertBuffer, meshTexBuffer, meshNormBuffer, colBuffer;
	Resources res;
	float meshVerts[], meshTex[], meshNormals[], pointsColor[];
	private float modelMatrix[];
	float enableLighting, enableTextureHandle;
	float enableColor = 0.0f;  // 0.0: obj   1.0: csv
...
//		meshTex = obj.getTexArray();
	...
	public void initBuffers(){
		Log.d("file_type", ""+RendererGL.file_type);
		if (!buffersInit) {
			...
			if(RendererGL.file_type == 1){
				colBuffer = ByteBuffer.allocateDirect(pointsColor.length * 4)
				.order(ByteOrder.nativeOrder())
				.asFloatBuffer();
				colBuffer.put(pointsColor).position(0);
				enableColor = 1.0f;
			}
...
//		int check = GLES20.glGetAttribLocation(mProgram, "vPosition");
		...
		if(RendererGL.file_type == 1){
			...
			GLES20.glVertexAttribPointer(mColorHandle, 4, GLES20.GL_FLOAT, false, 0, colBuffer);
		}
		//Normals
		...
		//uniform for enabling color
		GLES20.glUniform1f(
				GLES20.glGetUniformLocation(mProgram, "colorEnabled"), enableColor);
...
//												GLES20.GL_UNSIGNED_SHORT, 0);
			GLES20.glDrawArrays(GLES20.GL_POINTS, 0, meshVerts.length/COORDS_PER_VERTEX);
>>>>>>> parent of ec6abea... add some compat for android5.0+
